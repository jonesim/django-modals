...
class EditColumn(ModalLink):
    def __init__(self, modal_name, button_text='Edit', css_class='btn btn-sm btn-primary', title='Edit',
...
class DeleteColumn(ColumnBase):
    ...
    def __init__(self, modal_name,  field='id', button_text=None, css_class=None, no_col_search=True, **kwargs):
        if not self.initialise(locals()):
            return
        super().__init__(**self.kwargs)
        self.title = 'Delete'
